...
public class Route {
    ...
    public void turnOnAPi(final Double sLat, final Double sLng, final Double eLat, final Double eLng, final Context context, final TextView textView, final NaverMap naverMap) {
        ...
        odsayService.requestSearchPubTransPath(Double.toString(sLng), Double.toString(sLat), Double.toString(eLng),
                Double.toString(eLat), "0", "0", "0", new OnResultCallbackListener() {
                    ...
                    public void onSuccess(ODsayData odsayData, API api) {
                        ...
                        parseRoute(jsonObject, sLat, sLng, eLat, eLng, textView, context, naverMap);
                    }
    ...
    // 이동방법 파싱
    private void parseRoute(JSONObject jsonObject, Double sLat, final Double sLng, final Double eLat, final Double eLng,
                            TextView textView, Context context, NaverMap naverMap) {
        try {
            String routedetail = "";
...
// pathArray 안의 경로 갯수
            ...
            for (int i = 0; i < pathArraycount; i++) {
                ...
                textView.setText(routedetail);
                turnOnAPI2(mapObj, sLat, sLng, eLat, eLng, context, naverMap);
                break;
            }
    ...
    private void turnOnAPI2(String mapObj, final Double sLat, final Double sLng, final Double eLat, final Double eLng,
                            Context context, final NaverMap naverMap) {
        ...
        odsayService.requestLoadLane("0:0@" + mapObj, new OnResultCallbackListener() {
            ...
            public void onSuccess(ODsayData oDsayData, API api) {
                jsonObject = oDsayData.getJson();
                drawOnMap(jsonObject, sLat, sLng, eLat, eLng, naverMap);
            }
    ...
    // 경로 파싱
    private void drawOnMap(JSONObject jsonObject, Double sLat, final Double sLng, final Double eLat, final Double eLng, NaverMap naverMap) {
        ...
        try {
            ...
            for (int i = 0; i < graphPosCount; i++) {
                ...
                if (i == 0) {
                    seLatLngArray.add(new LatLng(sLat, sLng));
                ...
                if (i == graphPosCount - 1) {
                    seLatLngArray.add(new LatLng(lat, lng));
                    seLatLngArray.add(new LatLng(eLat, eLng));
